...
Compute fuzzy ranking values for each bacteria pathway combination using metabolic index generated by our laboratory's project (pending publication). The project can be obtained here: https://github.com/schandrababu/BMI.git
...
# 11. Create the Genomic Enzyme Tree and Function Tree
...
Analyze the intersect matrixes with Jaccard indexes, which are generated from Step13. The percentages calculated based on the frequency of the values in each intersect matrix are calculated based on six categories: "Values = 0", "Values > 0 AND <= 0.5", "Values > 0.5 AND < 1", "Values = 1", "have intersect and union length is 1", and "have intersect and union length is not 1".
...
Format the data based on two data files obtained in the previous steps for the visulaization purpose in the shiny application ProGut:
